...
Client.prototype._consume = function(){
	...
	this.channel.consume(this.callbackQueue, function(msg){
		...
		if( !self.calls[msg.properties.correlationId] ){
		...
		try {
			...
			self.channel.ack(msg);
			return self.calls[msg.properties.correlationId].reject(err);
		}

		self.calls[msg.properties.correlationId].resolve(result);
		self.channel.ack(msg);
	});
}
...
Client.prototype._genId = function(){
	...
	return this.nextcorrelationId.toString();
}
