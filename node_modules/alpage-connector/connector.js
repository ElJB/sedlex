...
Alpage.prototype.parse = function parse(){
	...
	this.request()
		.then(function(data){
			if( !data ){
				return deferred.resolve(new ParsedTree(""));
			}
...
Alpage.prototype.request = kirby.swallow(function request(){
	var deferred = Q.defer();
	this._post(deferred, 1);
	return deferred.promise;
});

Alpage.prototype._post = function(deferred, attempts){
	var self = this,
		maxAttempts = 2;

	console.log("Calling alpage demo: " + attempts);
	...
	req.on("response", function(res){
		...
		res.on("end", function(){
			if( !data.length && attempts < maxAttempts ){
				return self._post(deferred, attempts + 1);
			}
			if( !data.length ){
				console.log("Couldn't parse: " + self.sentence);
			}
