var Crawler = require("crawler"),
	connector = {},
	Q = require('q'),
	parseXml = require('xml2js').parseString,
	htmlparser = require("htmlparser2");

var statusMap = {
	'depot-au-parlement_projet': 2,
	'promulgation_proposition': 9,
	'promulgation-signature_ordonnance': 14,
	'examen_projet': 3,
	'promulgation_projet': 4,
	'conseil-des-ministres_projet': 1,
	'examen_proposition': 8,
	'conseil-ministres_ordonnance': 12,
	'decret-application_projet': 5,
	'decret-application_proposition': 10,
	'evaluation_projet': 6,
	'loi-ratification_ordonnance': 13,
	'depot-au-parlement_proposition': 7,
	'evaluation_proposition': 11
}

var htmlParser = new htmlparser.Parser({
    ontext: function(text){
        parseResult.content += text;
    },
    onend: function(){
    	parseResult.content = parseResult.content.replace("Où en est-on ?", "");
    	parseResult.content = parseResult.content.replace(/[\s]*Sur la toile publique[\s\S]*/, "");
    },
    onopentag: function(name, attribs){
        if(name === "a" && attribs.title.match("Dossier législatif")){
            parseResult.dossierUrl = attribs.href;
        }
    }
},
{
	decodeEntities: true
});
	
var parseResult = {
		content: "",
		dossierUrl: null 
	};

var parseHTML = function(html){
	parseResult = {
		content: "",
		dossierUrl: null 
	};
	htmlParser.write(html);
	htmlParser.end();
	htmlParser.reset();
	return parseResult;
}

var crawler = new Crawler({
		"maxConnections":10
	}),
		vpUri = "http://www.vie-publique.fr/spip.php?page=rdp_viepubliquefr_panorama_atom";

var entryToObj = function(entry){
	//TO DO: find a way to keep bullet lists in content
	//TO DO: include additional reading material as an attribute 
	var result = {};

	var keys = ["content", "summary"];
	for( i in keys ){
		result[keys[i]] = entry[keys[i]][0]._;
	}

	parseHTML(result.content);
	result.parliament_folder_url = parseResult.dossierUrl;
	result.content = parseResult.content;


	result.law_title = entry.title[0],
	result.date = entry.updated[0],
	result.url = entry.link[0].$.href;

	var procedure = connector.initiative(result.summary);
	result.procedure = procedure[0],
	result.author = procedure[1];

	result.tags = [];
	for( i in entry.category ){
		if( entry.category[i].$.term[0] == entry.category[i].$.term[0].toUpperCase() ){
			result.tags.push(entry.category[i].$.term);
		} else {
			if( !(result.status = statusMap[entry.category[i].$.term]) ){ throw new Error("Unknown law project status: " + entry.category[i].$.term); }
		}
	}
	if( !result.status ){
		result.status = 0;
	}

	return result;
}

connector.getSummaries = function(){
	return Q.promise(function(resolve, reject, notify){
		crawler.queue([{
			"uri": vpUri,
			"callback": function(err, result, $) {
				if( err ){ return reject(err); }
				parseXml(result.body, function(err, result){
					if( err ){ return reject(err); }
					try {
						result = result.feed.entry.map(entryToObj);
					} catch (e) {
						reject(e);
					}
					resolve(result);
					
				});
			}
		}]);
	});
}

connector.initiative = function initiative(summary){
	var projet = /projet de loi/.exec(summary)
	  , proposition = /proposition de loi/.exec(summary);

	if( projet && !proposition ){
		return ["projet", "Gouvernement"];
	} else if( !projet && proposition ){
		var mp = /M[\.me]+ ([ABCDEFGHIJKLMNOPQRSTUVWXYZ][abcdefghijklmnopqrstuvwxyz-]+ [ABCDEFGHIJKLMNOPQRSTUVWXYZ][abcdefghijklmnopqrstuvwxyz-]+)/.exec(summary)[1];

		return ["proposition", mp]
	} else {
		throw new Error("Can't identify law procedure");
	}
}

function log(e){
	console.log(e.stack);
}

module.exports = connector;