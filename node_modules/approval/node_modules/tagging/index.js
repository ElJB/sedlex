...
CLI.prototype.rating = function(){
	...
	console.log(chalk.green(util.format("Sentences rated: %s, positive: %s, negative: %s, useless: %s",
		self.current.rated_sentences,
		String(self.current.positive).substr(0, 4),
		String(self.current.negative).substr(0, 4),
		String(self.current.useless).substr(0, 4))));
...
var queryMap = {
	...
	'speeches': "SELECT _id, speech, rated_sentences, debate_section, positive, negative, useless FROM speech WHERE rated_sentences IS NOT NULL \
		AND rated_sentences > 0 AND rating IS NULL ORDER BY random()"
}
